...
- **"Equipped" has one definition** — `src/database/sql/equipped.ts` (`equippedSql` SQL predicate and its JS twin `isEquipped`/`tailEvidence`); fleet denominators come from `programmeRoster()` (`src/database/roster.ts`), except UA's headline total, which deliberately reads the sheet's figure (reasons in roster.ts's header); shared SQL in `sql/fragments.ts`, time windows in `sql/windows.ts`. Never re-derive either inline
