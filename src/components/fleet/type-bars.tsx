...
export function CarrierSection({
  ...
  const sorted = [...carriers].sort((a, b) => b.pct - a.pct || b.total - a.total);
  const listed = carriers.reduce((s, c) => s + c.total, 0);
  const regional = allTails.filter((t) => t.fleet === "express").length;
  const missing = regional - listed;
  return (
    <Section
    ...
    >
      <ul>
        {sorted.map((c) => (
          <ShareBarRow key={c.name} label={c.name} n={c.confirmed} total={c.total} />
        ))}
      </ul>
      {missing > 0 && (
        <p className="mt-3 text-xs text-muted">
          {fmt(missing)} of {fmt(regional)} regional aircraft have no operating carrier on record
          and aren't shown.
        </p>
      )}
    </Section>
