...
describe("/badge.svg", () => {
  ...
  // One question — "is a blended fleet percentage honest for this airline?" —
  // ended up with two independent answers. /badge.svg and the share cards gate
  // on rollout.rosterIsProgramScope; /airlines and /compare answer it by
  // rendering the per-type table wifiPhaseFamilies() returns INSTEAD of a
  // percentage. Nothing couples them, so an airline given a phase table but
  // left at rosterIsProgramScope: true would publish, cross-origin and
  // uncontexted, the exact blended number its own /compare table exists to
  // suppress. Until the two are one declaration, they must agree here.
  test("the badge's scope flag and the per-type table agree on every airline", () => {
    for (const cfg of Object.values(AIRLINES)) {
      const typeDetermined = wifiPhaseFamilies(cfg.code) !== null;
      expect(
        cfg.rollout.rosterIsProgramScope,
        `${cfg.code}: rosterIsProgramScope and wifiPhaseFamilies() disagree about whether a blended percentage is publishable`
      ).toBe(!typeDetermined);
    }
  });
